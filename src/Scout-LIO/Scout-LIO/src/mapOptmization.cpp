...
public:
    ...
    */
    mapOptimization()
    {
        ...
        // 订阅GPS里程计
        // sub_GPS_nav = nh.subscribe<sensor_msgs::NavSatFix>("/gps/fix", 100, &mapOptimization::GPS_callback, this, ros::TransportHints().tcpNoDelay());
    ...
    */
    ...
    void GPS2XYZ(double latitude, double longitude, double altitude, double* xyz)
    {
        ...
        //// 将经纬度转为 东北天坐标系下 里程计 xyz
        geoConverter.Forward(latitude, longitude, altitude, xyz[0], xyz[1], xyz[2]);
        //printf("la: %f lo: %f al: %f\n", latitude, longitude, altitude);
        //printf("gps x: %f y: %f z: %f\n", xyz[0], xyz[1], xyz[2]);
    }
    void GPS_callback(const sensor_msgs::NavSatFixConstPtr& gpsMSG)
    {
...
//        odomGPS.child_frame_id = "";
     ...
     ofstream foutC("/home/lab307/zqg/output/truth.txt", ios::app);            
      foutC.setf(ios::fixed, ios::floatfield);
      foutC.precision(10);
      foutC << odomGPS.header.stamp.toSec() << " ";
      foutC.precision(5);
      foutC << odomGPS.pose.pose.position.x << " "
        << odomGPS.pose.pose.position.y << " "
        << odomGPS.pose.pose.position.z << " "
        << odomGPS.pose.pose.orientation.w << " "
        << odomGPS.pose.pose.orientation.x << " "
        << odomGPS.pose.pose.orientation.y << " "
        << odomGPS.pose.pose.orientation.z << std::endl;
      foutC.close();
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    */
    bool saveMapService(lio_sam::save_mapRequest& req, lio_sam::save_mapResponse& res)
    {
        ...
        // {
        //   ROS_ERROR_STREAM("关键帧和时间戳数量不匹配");
        // }
        // else
        // {
        //   for (int i = 0; i < (int)cloudKeyPoses6D->size(); ++i)
        //   {
        //       ofstream foutC("/home/qigao/output/liosam_series.txt", ios::app);            
    ...
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        if (icp.hasConverged() == false || icp.getFitnessScore() > historyKeyframeFitnessScore)
            return;
        ROS_INFO_STREAM("===== loop closure converged and success  =====");
    ...
    bool detectLoopClosureDistance(int *latestID, int *closestID)
    {
        ...
        for (int i = 0; i < (int)pointSearchIndLoop.size(); ++i)
        {
            ...
            if (abs(copy_cloudKeyPoses6D->points[id].time - timeLaserInfoCur) > historyKeyframeSearchTimeDiff &&
                id < loopKeyCur - 30)
            {
                ROS_INFO_STREAM("===== id < loopKeyCur - 30 =====");
                loopKeyPre = id;
                break;
            }
        }
    ...
    void updateInitialGuess()
    {
        ...
        *************************** */
        ...
        if (cloudInfo.imuAvailable == true)
        {
            ...
            // 当前帧相对于前一帧的姿态变换
            //// lastImuTransformation 保存的是没来激光帧之前imu积出来的位姿，
            /// 这里求得的就是第一个帧时刻和第一帧imu时刻的位姿增量
    ...
    */
    void extractNearby()
    {
        ...
        //// 输入的是最近关键帧的位置信息，3DOF(XYZ), 最近邻匹配找到最近的关键帧
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0) {
                ...
                if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1)) {
                    ...
                    // area_012，也就是三个点组成的三角形面积*2，叉积的模|axb|=a*b*sin(theta)，写成叉乘的形式
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    */
    void transformUpdate()
    {
        ...
        ROS_WARN("transformTobeMapped%lf,%lf", transformTobeMapped[0], transformTobeMapped[1] );
...
//       ROS_INFO_STREAM("addGPSFactor...");
        ...
        if (aLoopIsClosed == true)
        {
            ...
            isam->update();
            isam->update();
            ROS_INFO_STREAM(" ISAM IS FINISHED !!");
        }
