...
public:
    ...
    */
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
      ...
      ofstream foutC("/home/lab307/zqg/output/liosam.txt", ios::app);            
      foutC.setf(ios::fixed, ios::floatfield);
      foutC.precision(10);
      foutC << imuOdomQueue.back().header.stamp.toSec() << " ";
      foutC.precision(5);
      foutC << laserOdometry.pose.pose.position.x << " "
...
public:
    ...
    */
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        ROS_WARN("imuPosecurPose %lf, %lf %lf \n",curPose.x(), curPose.y(), curPose.z());
        gtsam::PriorFactor<gtsam::Pose3> pose_factor(X(key), curPose, degenerate ? correctionNoise2 : correctionNoise);
        graphFactors.add(pose_factor);
        // 用前一帧的状态、偏置，施加imu预计分量，得到当前帧的状态
        gtsam::NavState propState_ = imuIntegratorOpt_->predict(prevState_, prevBias_);
        ROS_WARN("propState_ %lf, %lf %lf \n",propState_.t().x(), propState_.t().y(), propState_.t().z());
        // 变量节点赋初值
